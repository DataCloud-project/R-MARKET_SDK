...
const editRequestorder = async ({
	...
	iexecJson.order.requestorder.taskduration =
		taskduration || iexecJson.order.requestorder.taskduration;
	await saveJSONToFile(iexecJson, 'iexec.json');
};
...
describe('[Mainchain]', () => {
	...
	let mainchainServiceCatid = 5;
	let mainchainServiceDealid;
	let mainchainServiceTaskid;
	...
	test('[mainchain] iexec order sign --workerpool (service)', async () => {
		await editWorkerpoolorder({
		...
		expect(res.requestorder).toBeUndefined();
		expect(res.workerpoolorder.workerpool).toBeDefined();
		expect(res.workerpoolorder.category).toBe('5');
	});

	test('[mainchain] iexec order sign --request (service)', async () => {
		await editRequestorder({
			app: mainchainApp,
			dataset: mainchainDataset,
			workerpool: mainchainWorkerpool,
		...
		expect(res.requestorder).toBeDefined();
		expect(res.requestorder.app).toBeDefined();
		expect(res.requestorder.category).toBe('5');
	});

	test('[mainchain] iexec order fill (service)', async () => {
		await execAsync(`${iexecPath} order sign --app --raw`);
		await execAsync(`${iexecPath} order sign --dataset --raw`);
		...
		expect(tx).toBeDefined();
		expect(tx.gasPrice.toString()).toBe(chainGasPrice);
		mainchainServiceDealid = res.dealid;
	});
	...
	test('[mainchain] iexec deal show (service)', async () => {
		const raw = await execAsync(
			`${iexecPath} deal show ${mainchainServiceDealid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.deal).toBeDefined();
		...
		expect(res.deal.startTime).toBeDefined();
		expect(res.deal.finalTime).toBeDefined();
		expect(res.deal.duration).toBe('7200');
		expect(res.deal.deadlineReached).toBe(false);
		expect(res.deal.tasks).toBeDefined();
		expect(Object.keys(res.deal.tasks).length).toBe(1);
		expect(res.deal.tasks['0']).toBeDefined();
		mainchainServiceTaskid = res.deal.tasks['0'];
	});
	...
	test('[mainchain] iexec task show (service)', async () => {
		await initializeTask(tokenChainWallet, hubAddress, mainchainServiceDealid, 0);
		const raw = await execAsync(
			`${iexecPath} task show ${mainchainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.task).toBeDefined();
		expect(res.task.dealid).toBe(mainchainServiceDealid);
		expect(res.task.idx).toBe('0');
		expect(res.task.timeref).toBeDefined();
		expect(res.task.contributionDeadline).toBeDefined();
		expect(res.task.finalDeadline).toBeDefined();
	...
	test('[mainchain] iexec task show (extended service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task show ${mainchainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
	...
	test('[mainchain] iexec task interrupt (service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task interrupt ${mainchainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.txHash).toBeDefined();
	...
	test('[mainchain] iexec task show (interrupted service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task show ${mainchainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
...
describe('[Sidechain]', () => {
	...
	let sidechainServiceCatid = 5;
	let sidechainDealid;
	let sidechainServiceDealid;
	let sidechainTaskid;
	let sidechainServiceTaskid;
	...
	test('[sidechain] iexec order sign --workerpool (service)', async () => {
		await editWorkerpoolorder({
	...
	test('[sidechain] iexec order sign --request (service)', async () => {
		await editRequestorder({
			app: sidechainApp,
			dataset: sidechainDataset,
			workerpool: sidechainWorkerpool,
	...
	test('[sidechain] iexec order fill (service)', async () => {
		await execAsync(`${iexecPath} order sign --app --raw`);
		await execAsync(`${iexecPath} order sign --dataset --raw`);
		...
		expect(tx).toBeDefined();
		expect(tx.gasPrice.toString()).toBe(nativeChainGasPrice);
		sidechainServiceDealid = res.dealid;
	});
	...
	test('[sidechain] iexec deal show (service)', async () => {
		const raw = await execAsync(
			`${iexecPath} deal show ${sidechainServiceDealid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.deal).toBeDefined();
		...
		expect(Object.keys(res.deal.tasks).length).toBe(1);
		expect(res.deal.tasks['0']).toBeDefined();
		sidechainServiceTaskid = res.deal.tasks['0'];
	});
	...
	test('[sidechain] iexec task show (service)', async () => {
		await initializeTask(
			nativeChainWallet,
			nativeHubAddress,
			sidechainServiceDealid,
			0,
		);
		const raw = await execAsync(
			`${iexecPath} task show ${sidechainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.task).toBeDefined();
		expect(res.task.dealid).toBe(sidechainServiceDealid);
		expect(res.task.idx).toBe('0');
		expect(res.task.timeref).toBeDefined();
		expect(res.task.contributionDeadline).toBeDefined();
		expect(res.task.contributionDeadline).toBeDefined();
	...
	test('[sidechain] iexec task show (extended service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task show ${sidechainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
	...
	test('[sidechain] iexec task interrupt (service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task interrupt ${sidechainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
		expect(res.ok).toBe(true);
		expect(res.txHash).toBeDefined();
	...
	test('[sidechain] iexec task show (interrupted service)', async () => {
		const raw = await execAsync(
			`${iexecPath} task show ${sidechainServiceTaskid} --raw`,
		);
		const res = JSON.parse(raw);
