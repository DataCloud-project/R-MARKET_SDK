...
const hardwaredescriptionKeyName = {
	MIN_CPU: 'min_cpu',
	MAX_CPU: 'max_cpu',
	MIN_RAM: 'min_ram',
	MAX_RAM: 'max_ram',
...
const objHardwaredescriptionSchema = () =>
	object({
		[hardwaredescriptionKeyName.MIN_CPU]: number(),
		[hardwaredescriptionKeyName.MAX_CPU]: number(),
		[hardwaredescriptionKeyName.MIN_RAM]: number(),
		[hardwaredescriptionKeyName.MAX_RAM]: number(),
		[hardwaredescriptionKeyName.MIN_BW]: number(),
		[hardwaredescriptionKeyName.MAX_BW]: number(),
	}).noUnknown(true, 'Unknown key "${unknown}" in params');
...
const hardwaredescriptionSchema = () =>
	string()
		.transform((value, originalValue) => {
			if (typeof originalValue === 'object') {
				return JSON.stringify(originalValue);
	...
	object(
		{
			...
			hardwaredescription: hardwaredescriptionSchema(),
			volume: uint256Schema().required(),
			tag: tagSchema().required(),
